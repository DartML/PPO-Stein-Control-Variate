...
class NNValueFunction(object):
    ...
    def _build_graph(self): #NOTE:experimental results show that quartic function works also well
        """ Construct TensorFlow graph, including loss function, init op and train op """
        self.g = tf.Graph()
        with self.g.as_default():
            self.obs_ph = tf.placeholder(tf.float32, (None, self.obs_dim), 'obs_valfunc')
            self.val_ph = tf.placeholder(tf.float32, (None,), 'val_valfunc')
            # hid1 layer size is 10x obs_dim, hid3 size is 5, and hid2 is geometric mean
            ...
            val_params = tf.trainable_variables()
            val_dict = {}
            for var in val_params:
                print(var.name)
                val_dict[var.name] = var

            self.saver = tf.train.Saver(val_dict)
            # # Restore variables from disk.
            #self.saver = tf.train.Saver()
            #self.saver.restore(self.sess, "../data/value.ckpt.data-00000-of-00001")

            #self.saver = tf.train.import_meta_graph('./value.ckpt.meta')
            #self.saver.restore(self.sess, tf.train.latest_checkpoint('./'))

    def load_val_model(self, log_dir='log_dir/'):
        saver = tf.train.import_meta_graph(
                os.path.join(log_dir, 'val_models/',
                'value.ckpt.meta'))
    ...
    def save_val_func(self, model_dir="models/val_models"):
