...
from phi_functions.MLP_state import MLP_state_function
...
class Policy(object):
    ...
    def __init__(self, 
        ...
        if self.type == 'stein': # state-action control variate 
            self.phi = ContinousMLPPhiFunction(obs_dim, act_dim, 
                        hidden_sizes=phi_hidden_sizes, regular_scale=reg_scale)
        elif self.type == 'state': # state only control variate
            self.phi = MLP_state_function(obs_dim,
                        hidden_sizes=phi_hidden_sizes, regular_scale=reg_scale)
