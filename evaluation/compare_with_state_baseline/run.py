...
if __name__ == "__main__":
    ...
    parser.add_argument('-type', '--type', help='state only or state-action control variate: stein or state', 
                        type=str, default='stein')
    ...
    if args.load_model == 'save':
        ...
        if not os.path.exists('type=%s/max_timesteps=%s_eval_data'%(args.type, args.max_timesteps)):
            os.makedirs('type=%s/max_timesteps=%s_eval_data'%(args.type, args.max_timesteps))
